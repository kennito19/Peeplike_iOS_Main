...
class AlertVw: UIView, alertProtocol {
    ...
    func setBtnTitle(name:String) {
        btnDone.setTitle(name as String?, for: UIControl.State.normal)
    }
    ...
    func initialise(title:String, description:String, image:UIImage) {
        ...
        btnDone.setTitle("Okay", for: UIControl.State.normal)
        btnDone.addTarget(self, action: #selector(didButtonTapped), for: UIControl.Event.touchUpInside)
        btnDone.backgroundColor =  UIColor.darkText
        btnDone.setTitleColor(UIColor.groupTableViewBackground, for: UIControl.State.normal)
        btnDone.titleLabel?.font = UIFont(name: "Avenir Next Bold", size: 17)
        btnDone.backgroundColor = UIColor(red: 0.0/255.0, green: 136.0/255.0, blue: 118.0/255.0, alpha: 1.0) //t
